...
class Student:
    ...
    # This part is image labels setting start 
        ...
        f_lb1.place(x=0,y=0)
         ...
         # backgorund image 
        bg1=Image.open(r"images\pexels-gaurav-lodhi-4380162.jpg")
        bg1=bg1.resize((1537,768))
        self.photobg1=ImageTk.PhotoImage(bg1)

        # set image as label
        bg_img = Label(self.root,image=self.photobg1)
        bg_img.place(x=0,y=151,width=1537,height=768)
        ...
        main_frame.place(x=15,y=55,width=1503,height=580)

        # Left Label Frame 
        left_frame = LabelFrame(main_frame,bd=2,bg="white",relief=RIDGE,text="Student Details",font=("verdana",12,"bold"),fg="navyblue")
        left_frame.place(x=20,y=0,width=660,height=570)

        # Left Pic
        left_image= Image.open(r"images\down left Header.jpg")
        left_image= left_image.resize((635,150))
        photo = ImageTk.PhotoImage(left_image)
        left_image_label = Label(left_frame , image = photo , bg='#040405')
        left_image_label.image = photo
        left_image_label.place(x=10, y=0)
        ...
        current_course_frame.place(x=10,y=155,width=635,height=117)
        ...
        dep_combo["values"]=("Select Department","B.Sc","B.Com.","B.A.","M.Sc","M.Com.","M.A.")
        dep_combo.current(0)
        ...
        cou_combo["values"]=("Select Course","BCA","BIT","MCA","MIT","MBA","MATHS","PHY","CHEM","BBA","ACCOUNTS","BST","ECO","ENG","PSY")
        cou_combo.current(0)
        ...
        year_combo["values"]=("Select Year","2018-21","2019-22","2020-23","2021-24","2022-25")
        ...
        class_Student_frame.place(x=10,y=275,width=635,height=220)

        #Student id
        studentId_label = Label(class_Student_frame,text="Std-ID:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        studentId_label.grid(row=0,column=0,padx=5,pady=5,sticky=W)

        studentid = ttk.Entry(class_Student_frame,textvariable=self.var_std_id,width=15,font=("verdana",12,"bold"))
        studentid.grid(row=0,column=1,padx=5,pady=5,sticky=W)
        ...
        student_dob_entry = DateEntry(class_Student_frame,textvariable=self.var_dob,width=12,font=("verdana",13,"bold"))
        ...
        #Teacher Name
        student_tutor_label = Label(class_Student_frame,text="Teacher:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        ...
        btn_frame.place(x=10,y=500,width=635,height=45)

        #save button
        save_btn=Button(btn_frame,command=self.add_data,text="Save",width=9,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        save_btn.grid(row=0,column=0,padx=25,pady=5,sticky=W)

        #update button
        update_btn=Button(btn_frame,command=self.update_data,text="Update",width=9,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        update_btn.grid(row=0,column=1,padx=25,pady=5,sticky=W)

        #delete button
        del_btn=Button(btn_frame,command=self.delete_data,text="Delete",width=9,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        del_btn.grid(row=0,column=2,padx=25,pady=5,sticky=W)

        #reset button
        reset_btn=Button(btn_frame,command=self.reset_data,text="Reset",width=9,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        reset_btn.grid(row=0,column=3,padx=25,pady=5,sticky=W)

        #take photo button
        take_photo_btn=Button(class_Student_frame,command=self.generate_dataset,text="Take Pic",width=14,font=("verdana",8,"bold"),fg="white",bg="tomato")
        take_photo_btn.grid(row=5,column=3,padx=5,pady=0,sticky=W)

        #----------------------------------------------------------------------
        # Right Label Frame 
        right_frame = LabelFrame(main_frame,bd=2,bg="white",relief=RIDGE,text="Student Details",font=("verdana",12,"bold"),fg="navyblue")
        right_frame.place(x=715,y=0,width=760,height=570)

        # Right Pic
        right_image= Image.open(r"img2.jpg")
        right_image= right_image.resize((730,150))
        photo2 = ImageTk.PhotoImage(right_image)
        right_image_label = Label(right_frame , image = photo2 , bg='#040405')
        right_image_label.image = photo2
        right_image_label.place(x=10, y=0)
        ...
        search_frame.place(x=10,y=155,width=730,height=80)
        ...
        search_entry = ttk.Entry(search_frame,textvariable=self.var_search,width=18,font=("verdana",12,"bold"))
        ...
        search_btn.grid(row=0,column=3,padx=15,pady=10,sticky=W)

        showAll_btn=Button(search_frame,command=self.fetch_data,text="Show All",width=8,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        showAll_btn.grid(row=0,column=4,padx=15,pady=10,sticky=W)

        # -----------------------------Table Frame-------------------------------------------------
        #Table Frame 
        #Searching System in Right Label Frame 
        table_frame = Frame(right_frame,bd=2,bg="white",relief=RIDGE)
        table_frame.place(x=10,y=260,width=730,height=280)
        ...
        if self.var_dep.get()=="Select Department" or self.var_course.get=="Select Course" or self.var_year.get()=="Select Year" or self.var_semester.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_div.get()=="" or self.var_roll.get()=="" or self.var_gender.get()=="" or self.var_dob.get()=="" or self.var_email.get()=="" or self.var_mob.get()=="" or self.var_address.get()=="" or self.var_teacher.get()=="":
            ...
            try:
                ...
                if Update > 0:
                    conn = mysql.connector.connect(user='root', password='root',host='localhost',database='face_recognition',port=3306)
                    mycursor = conn.cursor()
                    mycursor.execute("update student set Name=%s,Department=%s,Course=%s,Year=%s,Semester=%s,Division=%s,Gender=%s,DOB=%s,MobNo=%s,Address=%s,RollNo=%s,Email=%s,Teacher=%s,PhotoSample=%s where studentid=%s",( 
                    self.var_std_name.get(),
                    self.var_dep.get(),
                    self.var_course.get(),
                    self.var_year.get(),
    ...
    def search_data(self):
        if self.var_search.get()=="" or self.var_searchTX.get()=="Select":
            ...
            try:
                ...
                sql = "SELECT studentid,Name,Department,Course,Year,Semester,Division,Gender,DOB,MobNo,Address,RollNo,Email,Teacher,PhotoSample FROM student where Name='" +str(self.var_search.get()) + "'"
